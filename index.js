// /var/www/liberalizm.me/index.js (FİNAL PERFORMANS v2: CACHING + ODA SİSTEMİ)
...
// Güvenlik ve yardımcı fonksiyonlar (değişiklik yok)
...
// Uygulama kurulumu (değişiklik yok)
const app = express();
...
// YENİ: PERİYODİK ÖNBELLEK GÜNCELLEME FONKSİYONU
async function updateOnlineUsersCache() {
    if (!redisClient.isReady) return;
    try {
        const onlineKeys = await redisClient.sMembers('online_users_set');
        if (onlineKeys.length === 0) {
            await redisClient.set('online_users_cache', '[]', { EX: 15 });
            return;
        }
        const multi = redisClient.multi();
        for (const key of onlineKeys) { multi.hGetAll(`user:${key}`); }
        const onlineUsersDataRaw = await multi.exec();
        const finalOnlineList = onlineUsersDataRaw.filter(user => user && user.publicKey).map(user => ({ username: user.username, publicKey: user.publicKey }));
        await redisClient.set('online_users_cache', JSON.stringify(finalOnlineList), { EX: 15 }); // 15 saniye geçerli cache
    } catch (error) {
        console.error("Online kullanıcı önbelleği güncellenirken hata:", error);
    }
}
...
io.on('connection', async (socket) => {
    socket.on('add user', async (userData) => {
        try {
            ...
            socket.username = userData.username; socket.publicKey = userData.publicKey;
            socket.join(userData.publicKey);
            socket.join(GENERAL_CHAT_ROOM);

            await redisClient.sAdd('online_users_set', userData.publicKey);
            await usersCollection.updateOne({ publicKey: userData.publicKey }, { $set: { username: userData.username } }, { upsert: true });
            const userKey = `user:${socket.publicKey}`;
            await redisClient.hSet(userKey, { username: userData.username, publicKey: userData.publicKey, socketId: socket.id });
            await redisClient.expire(userKey, 300);

            // ==============================================================================
            // FİNAL OPTİMİZASYONU: HAZIR ÖNBELLEĞİ KULLAN
            // ==============================================================================
            const cachedList = await redisClient.get('online_users_cache');
            socket.emit('initial user list', cachedList ? JSON.parse(cachedList) : []);

            socket.to(GENERAL_CHAT_ROOM).emit('user connected', { username: userData.username, publicKey: userData.publicKey });

            // Anlık olarak cache'i güncellemek için tetikle (isteğe bağlı ama anlık his verir)
            updateOnlineUsersCache();

        } catch (error) { console.error('[HATA] "add user":', error); }
    });
    ...
    socket.on('disconnect', async () => { 
        if (socket.publicKey) { 
            ...
            io.to(GENERAL_CHAT_ROOM).emit('user disconnected', { publicKey: socket.publicKey }); 
            updateOnlineUsersCache(); // Cache'i anlık güncelle
        } 
    });
    
    socket.on('chat message', async (msg, callback) => { 
        if (!socket.username || !msg.message) return; 
        const expireDate = new Date(); expireDate.setSeconds(expireDate.getSeconds() + 86400); 
        ...
        try { 
            ...
            socket.to(GENERAL_CHAT_ROOM).emit('chat message', data);
            if (typeof callback === 'function') callback({ status: 'ok' }); 
        } catch (err) { if (typeof callback === 'function') callback({ status: 'error' }); } 
    });

    // Diğer eventlerde büyük değişiklik yok
...
// Sunucu başladığında ve her 5 saniyede bir cache'i güncelle
